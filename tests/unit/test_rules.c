...
static void test_rule_engine_create(void **state)
{
    ...
    SwlRuleEngine *engine = swl_rule_engine_create();
    assert_non_null(engine);
    assert_int_equal(swl_rule_engine_count(engine), 0);
    swl_rule_engine_destroy(engine);
}
...
static void test_rule_engine_add(void **state)
{
    ...
    SwlError err = swl_rule_engine_add(engine, &rule);
    assert_int_equal(err, SWL_OK);
    assert_int_equal(swl_rule_engine_count(engine), 1);
...
static void test_rule_engine_add_null_checks(void **state)
{
    ...
    assert_int_equal(swl_rule_engine_add(NULL, &rule), SWL_ERR_INVALID_ARG);

    SwlRuleEngine *engine = swl_rule_engine_create();
    assert_int_equal(swl_rule_engine_add(engine, NULL), SWL_ERR_INVALID_ARG);
    swl_rule_engine_destroy(engine);
}
...
static void test_rule_engine_add_multiple(void **state)
{
    ...
    for (size_t i = 0; i < 3; i++) {
        SwlError err = swl_rule_engine_add(engine, &rules[i]);
        assert_int_equal(err, SWL_OK);
    }

    assert_int_equal(swl_rule_engine_count(engine), 3);
...
static void test_rule_engine_get(void **state)
{
    ...
    swl_rule_engine_add(engine, &rule);

    const SwlRule *got = swl_rule_engine_get(engine, 0);
    assert_non_null(got);
...
static void test_rule_engine_get_out_of_bounds(void **state)
{
    ...
    SwlRule rule = { .app_id_pattern = "test" };
    swl_rule_engine_add(engine, &rule);

    assert_null(swl_rule_engine_get(engine, 1));
    assert_null(swl_rule_engine_get(engine, 100));
    assert_null(swl_rule_engine_get(NULL, 0));
...
static void test_rule_engine_remove(void **state)
{
    ...
    for (size_t i = 0; i < 3; i++) {
        swl_rule_engine_add(engine, &rules[i]);
    }

    assert_int_equal(swl_rule_engine_count(engine), 3);

    /* Remove middle rule */
    SwlError err = swl_rule_engine_remove(engine, 1);
    assert_int_equal(err, SWL_OK);
    assert_int_equal(swl_rule_engine_count(engine), 2);

    /* Check remaining rules */
    assert_string_equal(swl_rule_engine_get(engine, 0)->app_id_pattern, "first");
    assert_string_equal(swl_rule_engine_get(engine, 1)->app_id_pattern, "third");
...
static void test_rule_engine_remove_invalid_index(void **state)
{
    ...
    SwlError err = swl_rule_engine_remove(engine, 5);
    assert_int_equal(err, SWL_ERR_INVALID_ARG);
    assert_int_equal(swl_rule_engine_count(engine), 1);

    err = swl_rule_engine_remove(NULL, 0);
    assert_int_equal(err, SWL_ERR_INVALID_ARG);
...
static void test_rule_engine_clear(void **state)
{
    ...
    swl_rule_engine_clear(engine);
    assert_int_equal(swl_rule_engine_count(engine), 0);

    /* Can still add after clear */
    swl_rule_engine_add(engine, &rules[0]);
    assert_int_equal(swl_rule_engine_count(engine), 1);
...
static void test_rule_engine_count_null(void **state)
{
    ...
    assert_int_equal(swl_rule_engine_count(NULL), 0);
}
...
static void test_rule_regex_app_id(void **state)
{
    ...
    /* Regex patterns */
    SwlRule rule1 = { .app_id_pattern = "^firefox.*$" };
    SwlRule rule2 = { .app_id_pattern = ".*terminal.*" };

    swl_rule_engine_add(engine, &rule1);
    swl_rule_engine_add(engine, &rule2);

    /* Verify patterns were stored */
    const SwlRule *got = swl_rule_engine_get(engine, 0);
    assert_string_equal(got->app_id_pattern, "^firefox.*$");

    got = swl_rule_engine_get(engine, 1);
...
static void test_rule_remove_first(void **state)
{
    ...
    swl_rule_engine_add(engine, &rules[0]);
    swl_rule_engine_add(engine, &rules[1]);

    swl_rule_engine_remove(engine, 0);
    assert_int_equal(swl_rule_engine_count(engine), 1);
    assert_string_equal(swl_rule_engine_get(engine, 0)->app_id_pattern, "second");
...
static void test_rule_remove_last(void **state)
{
    ...
    swl_rule_engine_remove(engine, 1);
    assert_int_equal(swl_rule_engine_count(engine), 1);
    assert_string_equal(swl_rule_engine_get(engine, 0)->app_id_pattern, "first");
